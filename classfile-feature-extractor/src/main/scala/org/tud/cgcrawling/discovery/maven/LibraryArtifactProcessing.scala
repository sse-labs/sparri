...
import de.tudo.classfilefeatures.common.download.HttpDownloader
...
trait LibraryArtifactProcessing {
  ...
  def createIdentifierIterator(groupId: String, artifactId: String): Try[Iterable[MavenIdentifier]] = {
    getVersions(groupId, artifactId)
      .map(versions => versions.map(version => new MavenIdentifier(groupId, artifactId, version, repoUri.toString)))
  }
