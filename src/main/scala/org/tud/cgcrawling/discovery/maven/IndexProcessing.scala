...
class MavenIndexReader(base: URL) extends AppLogging{
  ...
  def read(): Option[MavenIdentifier] = {
    ...
    def readInternal(kvp: util.Map[String, String]) = {
      ...
      identifierAttempt match {
        ...
        case Failure(e) => {
          log.warning(s"While processing index we received the following u-value that we could not split $kvpU. Full kvp is $kvp. Exception was $e.")
          None
        }
      }
    }
