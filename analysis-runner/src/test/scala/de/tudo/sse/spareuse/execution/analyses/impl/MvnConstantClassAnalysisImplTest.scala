...
class MvnConstantClassAnalysisImplTest extends AnyFlatSpec with must.Matchers {
  ...
  "The constant class analysis" must "calculate valid results" in {
    ...
    buildClassFor(samplePackage1, "test", "test1/test", isInterface = false, hash = Array(3.byteValue(), 4.byteValue()))
    buildClassFor(samplePackage2, "test", "test1/test", isInterface = false, hash = Array(3.byteValue(), 4.byteValue()))
    buildClassFor(samplePackage3, "test", "test1/test", isInterface = false, hash = Array(2.byteValue(), 4.byteValue()))
