      ...
      case Success(dependencies) =>
        ...
        val dependenciesMissingResults = dependencies
          .filter{ dep =>
            ...
            eval.getRunsForEntity(eid, IFDSTaintFlowSummaryBuilderImpl.analysisName, "0.0.5", eval.getApiBaseUrl, httpClient) match {
        ...
        if(dependenciesMissingResults.nonEmpty){
          dependenciesMissingResults.foreach{ dep =>
            eval.triggerAnalysisRun(Set(toEID(dep.identifier)), IFDSTaintFlowSummaryBuilderImpl.analysisName, "0.0.5", eval.getApiBaseUrl, httpClient) match {
              case Success(runId) =>
