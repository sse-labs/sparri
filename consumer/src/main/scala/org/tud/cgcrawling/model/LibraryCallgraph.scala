...
class LibraryCallgraph {
  ...
  def addMethod(method: Method, isExternal: Boolean): Unit = {
    val newMethod = new LibraryMethod(MethodIdentifier.fromOpalMethod(method, isExternal))
  ...
  def addEdge(caller: Method, callee: Method, calleeIsExternal: Boolean): Unit = {
    ...
    val newCallee = new LibraryMethod(MethodIdentifier.fromOpalMethod(callee, calleeIsExternal))
