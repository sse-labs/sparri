...
final case class PullLookupRequestsResponse(isResolving: Boolean, requests: Set[LookupRequestRepresentation], hasFailed: Boolean, fatalError: Option[String])
...
trait PullLookupRequestsResponseJsonSupport extends SprayJsonSupport with DefaultJsonProtocol {
  ...
  implicit val pullResponseFormat: JsonFormat[PullLookupRequestsResponse] = jsonFormat4(PullLookupRequestsResponse)
}
